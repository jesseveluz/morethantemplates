...
$admincontent .= '<div style="margin: 0px; font: normal 14px verdana; background-color: #FFF; width:100%"><table width="100%"><tr><td><div class="adminsubmenu"><a href="'.$program_url.'/admin/cp/forumsectionadd.php">Add Forum Section</a></div></td></tr></table></div>';
...
function querylist($orderby='',$offset=0,$items=10){
	...
	$query = 'SELECT  t1.f_ID,f_sectionname,f_description,t1.f_topics,f_moderatorID,t2.f_firstname,t2.f_lastname,t1.f_status from '.$dbprefix_.'forumsections as t1 left outer join '.$dbprefix_.'members as t2 on t1.f_moderatorID=t2.f_ID
 '.$order;
	
	$csvquery = 'SELECT  t1.f_ID,f_sectionname,f_description,f_topics,f_moderatorID,t2.f_firstname,t2.f_lastname,t1.f_status from '.$dbprefix_.'forumsections as t1 left outer join '.$dbprefix_.'members as t2 on t1.f_moderatorID=t2.f_ID ';
	
	$countquery = 'SELECT  f_ID from '.$dbprefix_.'forumsections';
