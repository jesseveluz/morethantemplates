...
if (isset($_GET['t']) && isset($_GET['pg']) && !isset($_GET['p']) && !isset($_GET['act'])){
		...
		if (isset($_COOKIE[md5("adminname".__SECRET_WORD__)]) || $moderatorID==$memberid){
			$query = "select f_postID from ".$dbprefix_."forumposts where f_postforumID=".$_GET['t']." and f_postreplyto='0' and f_poststatus!='deleted' ";
		...
		if ($item['f_poststatus']=='pending'){
			if ($item['f_postauthorID']==$memberid){
				...
				if (isset($_COOKIE[md5("adminname".__SECRET_WORD__)]) || $moderatorID==$memberid){
					...
					$rowdata['col1'] = '<h3><a href="?t='.$_GET['t'].'&pg='.$_GET['pg'].'&p='.$item['f_postID'].'">'.cleanuptext($item['f_posttitle']).'</a></h3>';
					$rowdata['col1'] .= '<div style="padding: 10px; background: #ffffcc;"><strong>PENDING:</strong> &nbsp; <a href="?t='.$_GET['t'].'&pg='.$_GET['pg'].'&p='.$item['f_postID'].'&act=publishtopic&cd='.md5($item['f_postID']).'">Publish</a> &nbsp; | &nbsp; <a href="?t='.$_GET['t'].'&pg='.$_GET['pg'].'&p='.$item['f_postID'].'&act=removetopic&cd='.md5($item['f_postID']).'">Remove</a></div>';
