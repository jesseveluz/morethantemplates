...
if ($_GET['act']=='getlist'){
	...
	// header part
	$rowdata['col1'] = '<a href="?act='.$_GET['act'].'&orderby=f_firstname&sortorder='.$sortorder.'">Firstname</a>';
	$rowdata['col2'] = '<a href="?act='.$_GET['act'].'&orderby=f_lastname&sortorder='.$sortorder.'">Lastname</a>';
	$rowdata['col3'] = '<a href="?act='.$_GET['act'].'&orderby=f_email&sortorder='.$sortorder.'">Email</a>';
	$rowdata['col4'] = '<a href="?act='.$_GET['act'].'&orderby=f_dateconfirmed&sortorder='.$sortorder.'">Confirmed</a>';
	$rowdata['col5'] = '<a href="?act='.$_GET['act'].'&orderby=f_password&sortorder='.$sortorder.'">Password</a>';
	$rowdata['col6'] = '<a href="?act='.$_GET['act'].'&orderby=f_country&sortorder='.$sortorder.'">Country</a>';
	$rowdata['col7'] = '<a href="?act='.$_GET['act'].'&orderby=f_status&sortorder='.$sortorder.'">Status</a>';
	$rowdata['col8'] = 'Action';
	...
	while ($item=@mysql_fetch_assoc($queryresource)){
		$rowdata['col1'] = ucwords($item['f_firstname']);
		$rowdata['col2'] = ucwords($item['f_lastname']);
		$rowdata['col3'] = $item['f_email'];
		$rowdata['col4'] = $item['f_dateconfirmed'];
		$rowdata['col5'] = $item['f_password'];
		$rowdata['col6'] = $item['f_country'];
		$mstatus = 'free';
		if ($item['f_status']=='1'){
			$mstatus = 'premium';
		}
		$rowdata['col7'] = $mstatus;
		$rowdata['col8'] = '<a href="memberdetails.php?fid='.$item['f_ID'].'">Details</a> | <a href="removemember.php?fid='.$item['f_ID'].'">Remove</a>';
...
AND f_confirmed = \'Y\' and (f_firstname like \'%'.$_GET['word'].'%\' or f_lastname like \'%'.$_GET['word'].'%\' or f_email like \'%'.$_GET['word'].'%\') '.$order;
...
WHERE f_dateconfirmed between \''.$_SESSION['startdate'].' 00:00:00\' AND \''.$_SESSION['endingdate'].' 23:59:59\' 
AND f_confirmed = \'Y\'  and (f_firstname like \'%'.$_GET['word'].'%\' or f_lastname like \'%'.$_GET['word'].'%\' or f_email like \'%'.$_GET['word'].'%\') ';
	
	$countquery = 'SELECT  f_email from '.$dbprefix_.'members WHERE f_dateconfirmed between \''.$_SESSION['startdate'].' 00:00:00\' AND \''.$_SESSION['endingdate'].' 23:59:59\' AND f_confirmed = \'Y\'  and (f_firstname like \'%'.$_GET['word'].'%\' or f_lastname like \'%'.$_GET['word'].'%\' or f_email like \'%'.$_GET['word'].'%\') ';
