...
$pagemessage = '<strong>Are you sure you want to remove this section?</strong><br /><br />';
...
$programtitle = 'Forum Manager > Removing Section';
...
$section_isremoved = 'false';

if (isset($_POST['submit'])){
	if ($_POST['submit']=='Yes'){
		$query = 'delete from '.$dbprefix_.'forumsections where f_ID=\''.$_GET['fid'].'\' ';
		$queryresource = @mysql_query($query, CONNECTION);
		if ($queryresource){
			$pagemessage = '<div style="background: #ffffcc; padding: 10px;">Section was removed successfuly</div>';
			$section_isremoved = 'true';
		} else {
			$pagemessage = '<div style="background: #ffcccc; padding: 10px;">Oops! section is not removed</div>';
		}
	} else	if ($_POST['submit']=='No'){
		header('location: forumsectionedit.php?fid='.$_GET['fid'].'');
		exit;
	} else {
	}
}

if ($section_isremoved=='false'){
	...
	if (@mysql_num_rows($queryresource)){
		...
		$admincontent .= '<tr valign="top"><td>Section Name:</td><td><input name="sectionname" value="'.$item['f_sectionname'].'"  readonly="readonly" /></td></tr>';
		$admincontent .= '<tr valign="top"><td>Description:</td><td><input name="description" value="'.$item['f_description'].'"  readonly="readonly" /></td></tr>';
		$admincontent .= '<tr valign="top"><td>Topics:</td><td><input name="topics" value="'.$item['f_topics'].'" readonly="readonly" /></td></tr>';
		$admincontent .= '<tr valign="top"><td>Moderator ID:</td><td><input name="moderatorid" value="'.$item['f_moderatorID'].'"  readonly="readonly" /></td></tr>';
		$admincontent .= '<tr valign="top"><td>Status:</td><td><input name="status" value="'.$item['f_status'].'"  readonly="readonly" /></td></tr>';
		$admincontent .= '<tr valign="top"><td><hr /></td><td><hr /></td></tr>';
		$admincontent .= '<tr valign="top"><td></td><td><input type="submit" name="submit" value="No" /> &nbsp; &nbsp; <input type="submit" name="submit" value="Yes" /></td></tr>';
		$admincontent .= '</table>';
		$admincontent .= '</form>';
	} else {
		$admincontent = '';
